...
#define BATT_METER_INIT_ERR_CNT         3
#define BATT_METER_VERIFY_CNT           20
...
static void batt_meter_event_handler(void* p_ctx)
{
    ...
    {
        app_timer_start(batt_meter_event_id, 100, NULL);
        return;
    }
    ...
    else if(battMeterStatus==BATT_METER_VERIFY_CONFIG)
    {
        ...
        if(get_battery_init_status()!=APP_DRV_SUCCESS)
        {
            ...
            if(battMeterVerifyCnt<BATT_METER_VERIFY_CNT)
            {
                ...
                app_timer_start(batt_meter_init_event_id,(battMeterVerifyCnt<4)?1000:100,NULL);
                return;
            }
            else
            {
...
void batt_meter_init_event_register(void)
{
    ...
    batt_meter_init_event_start(50);
    logX("</%s >", __func__);
}
...
static void batt_charge_int_event_handler(void* p_ctx)
{
    ...
    {
        LOG("wlc busy");
        app_timer_start(batt_charge_int_event_id, 1000, NULL);
        // 无线充数据传输完成之前不可以关闭TX
        return;
    }
...
static void batt_charge_init_event_handler(void* p_ctx)
{
    if (MP2662_Init() != APP_DRV_SUCCESS)
    {
        ...
        {
            app_timer_start(batt_charge_init_event_id, 100, NULL);
        }
        else
        {
            battChargeStatus = BATT_CHARGEING_ABNORMAL;
            APP_LOG_INFO("charge abnormal");
            g_sensor_init_event_start(100); //charge ic 异常允许触发广播排除故障
        }
    }
    else
    {
